...
import { ALL_SUPPORTED_CHAIN_ID_NUMBERS } from "../utils/constants/chains";
...
const useReserveList = (poolAddress: any, recheckReserver: boolean) => {
  ...
  console.log(
    "chainid",
    ALL_SUPPORTED_CHAIN_ID_NUMBERS.includes(chainId as number)
  );
    ...
    if (poolAddress && account) {
      try {
        if (ALL_SUPPORTED_CHAIN_ID_NUMBERS.includes(chainId as number)) {
          ...
          const tokenSymbolData: [number, string[]] = await multi.multiCall(
            erc20TokenAbi,
            tokenSymbolInputs
          );
          ...
          for (let i = 0; i < list.length; i++) {
            // const coin = tokenList.data.filter(
            //   (coin: any) => coin.symbol == tokenSymbolData[1][i].toLowerCase()
            // );

            const symbol: string = tokenSymbolData[1][i];

            const logo = LOGO[symbol];
